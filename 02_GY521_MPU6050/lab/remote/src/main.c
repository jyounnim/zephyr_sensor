/*
 * main.c - 02_GY521_MPU6050 (core1 / appcpu image)
 *
 * Same pattern as Lab 01's core1: receive sensor snapshots from core0
 * over IPM and draw them on the SSD1306. This core never touches I2C0
 * or the MPU6050 - see src/main.c (core0) and the design doc for why
 * the two cores each own a different physical I2C controller.
 *
 ...
 * arrived.
 *
 * core0 pushes immediately when the accel or gyro vector magnitude
 * moves past a threshold since the last value it sent, plus a 1s
 * heartbeat so this side has something to declare "alive" during
 * quiet periods. This side only cares about elapsed time since the
 * last arrival: if nothing shows up in `sensor_msgq` for 2 s, the
...
/* Depth 4: core0 sends at most every 100 ms in the busiest case
 * (constant motion), so this just needs enough slack to absorb a
 * brief scheduling delay on the display thread.
 */
			  ...
			  uint32_t id, volatile void *data)
{
	...
	if (k_msgq_put(&sensor_msgq, &payload, K_NO_WAIT) != 0) {
		/* Queue momentarily full - drop the oldest entry and
		 * keep the newest one, since only the latest snapshot is
		 * ever useful for a live display.
		 */
...
static void render_payload(const struct ipm_sensor_payload *p, bool stale)
{
	...
	if (p->valid) {
		...
		format_fixed(p->gyro_x_dps, 1, num, sizeof(num));
		snprintf(line, sizeof(line), "GX:%s", num);
		ssd1306_draw_text(4, 0, line);

		format_fixed(p->gyro_y_dps, 1, num, sizeof(num));
		snprintf(line, sizeof(line), "GY:%s", num);
		ssd1306_draw_text(5, 0, line);

		format_fixed(p->gyro_z_dps, 1, num, sizeof(num));
		snprintf(line, sizeof(line), "GZ:%s", num);
		ssd1306_draw_text(6, 0, line);

		format_fixed(p->temp_c, 1, num, sizeof(num));
		snprintf(line, sizeof(line), "TEMP:%sC", num);
		ssd1306_draw_text(7, 0, line);
	} else {
		ssd1306_draw_text(3, 0, "MPU6050 ---");
	}
...
static void display_thread_fn(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		 ...
		 * every 1 s even with no motion, so 2000 ms of total
		 * silence means something upstream stalled (sensor
		 * thread stuck, IPM channel jammed, core0 crashed, etc)
		 * rather than just "nothing moved".
		 */
...
int main(void)
{
	LOG_INF("02_GY521_MPU6050 (core1/appcpu) starting");
